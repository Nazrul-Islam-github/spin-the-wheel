    ...
    // 
        for (let y = 1; y <= this.numSegments; y++) {
            if (this.segments[y].image !== null) {
                LoadAllImages(this.segments[y]).then(()=>{
                    this.draw(this.clearCanvas);
                }).catch(error=>{
                    console.log(error);
                })
            ...
            if (seg.text) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (direction == 'reversed') {
                        ...
                        if (orientation == 'horizontal') {
if(seg.image && !this.notshowimage){
    if(seg.imgData){      
        // console.log(seg.imgData);           
        ...
        this.ctx.rotate(textAngle);
        this.ctx.translate(-centerX, -centerY);
let marginFromCenter = 2;

if(this.segments.length>6){
    let diff = this.segments.length - 6;

    for(let i = 0;i<diff;i++){
        marginFromCenter = marginFromCenter+.90;
        if(seg.imageWidth<36){
            // break;
        }
    }
}
        ...
        this.ctx.drawImage(seg.imgData, centerX + innerRadius + ((outerRadius - innerRadius) / 2) + margin+marginFromCenter, centerY-(seg.imageHeight/2) ,seg.imageWidth,seg.imageHeight );

        // positio center minus by half of image height
...
Winwheel.prototype.addSegment = function (options, position) {
    ...
    if (typeof position !== 'undefined') {
        ...
        for (let x = this.numSegments; x > position; x--) {
            ...
            if (this.segments[x].image !== null) {
                this.segments[x].imgData = new Image();
                this.segments[x].imgData.onload = winwheelLoadedImage;
                this.segments[x].imgData.src = this.segments[x].image;
            }
        }
...
function Segment(options) {
    ...
    let defaultOptions = {
        ...
        'fillStyle': "transparent", // If null for the rest the global default will be used.
        'strokeStyle': null,
        'lineWidth': null,
        'textFontFamily': null,
        'textFontSize': null,
...
async function winwheelLoadedImage(callback,e,segment) {
    // this is for image fit image in segments
  
    const oWidth = e.target.naturalWidth;
    const oHeight = e.target.naturalHeight;
    const aspectRatio = Math.round((oHeight / oWidth) * 100);
    let width = 120
   const segmentsLength = winwheelToDrawDuringAnimation.segments.length;
    if(segmentsLength>4){
        let d = segmentsLength-8;
 for(let i =0;i<d;i++){
    width = width-6;
 }
 ...
 if(width<35){
    ...
    let newWidth = width*winwheelToDrawDuringAnimation.scaleFactor;
   segment.imageWidth =newWidth,
   segment.imageHeight =   Math.round(
    (newWidth / 100) * aspectRatio
  );


//    this callback is for draw image in canvas when is image onload is complete;
  callback()
...
const LoadAllImages = (imageSegments)=>{
    return new Promise( (resolve,reject)=>{
        imageSegments.imgData = new Image();
// imageSegments.imgData.width = 50
        imageSegments.imgData.onload = (e)=>winwheelLoadedImage(resolve,e,imageSegments);
        
        imageSegments.imgData.onabort= (error)=>reject(error.mesage)
        imageSegments.imgData.src = imageSegments.image;
